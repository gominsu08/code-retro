...
최종 갱신: 2026-09-17. **로컬 MVP 검증 완료, 공개 배포 준비** 단계다.
...
- 로컬 확인용 버전을 제공한 뒤 **최신 요청: “그러면 이제 배포를 위한 준비를 하자 내가 어떤걸 해줘야해?”**를 받았다. Render·Neon 계정 준비와 새 GitHub 저장소의 이름·공개 여부를 결정한 뒤 배포를 진행한다. 계정 생성·원격 업로드·공개 서비스 생성은 아직 하지 않았다.
...
| M7 운영·배포 | 구성 준비 / 배포 미진행 | Alembic, Dockerfile, Render 구성. PostgreSQL·Linux 컨테이너·외부 접속은 미검증 |
...
- `pytest`: 22개 통과. CSRF·세션 격리, GitHub URL·blob, 구문 분석, 설정 충돌·중복 요청, lease, 코멘트·출력, Gemini 도구 왕복·기존 인용 재사용 검증.
...
- SQLite Alembic upgrade/check 통과. 서버 재시작 후 저장 데이터 유지 확인. 실제 PostgreSQL 경쟁 상황·컨테이너 종료 복구는 추가 검증 대상이다.
...
6. 삭제 API는 작업 취소와 숨김 처리다. 만료 자료 영구 정리, 백업 보존 기간, IP별 제한, worker 운영 지표는 공개 운영 전에 보완한다.
7. 공개 배포·별도 기기 접속, 사용자 후기, 블로그·SNS 게시, 제출용 GitHub 저장소는 아직 완료되지 않았다.

## 다음 세션에서 할 일

사용자의 로컬 화면 피드백을 먼저 반영한다. 공개 배포를 다시 요청하면 [배포 메모](DEPLOYMENT.md)의 미검증 항목을 확인하고 무료 계정을 연결한다. 가입·약관 동의는 사용자가 직접 한다. 이미 승인한 Gemini 무료 사용 방침을 다시 묻거나 유료 제공자로 자동 변경하지 않는다.
