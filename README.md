...
2. **분석 범위**에서 폴더를 펼쳐 원하는 위치로 이동합니다. 폴더 체크박스는 하위 파일을 함께 선택·해제하며, 개별 파일과 분류·담당 범위도 조정할 수 있습니다. 작은 화면에서도 같은 폴더 탐색기를 사용합니다.
3. **시스템 분석 시작**을 누릅니다. 기본값은 코드 구조 분석이며, 필요한 시스템에서 나중에 AI 설명을 요청할 수 있습니다.
4. **시스템 회고**의 왼쪽 목록에서 `Machine` 같은 시스템을 고르고 **AI 설명 생성**을 누릅니다. 사용한 방식과 이유, 핵심 구현을 짧은 회고 문단으로 정리합니다. 기존 AI 설명은 **AI 설명 다시 쓰기**로 새 버전을 만들 수 있습니다. 확인되지 않은 의도는 단정하지 않으며 이전 버전과 이미 인용한 코멘트 근거를 보존합니다.
...
- 공개 C# 파일 목록, 중간 폴더를 포함한 트리 탐색, 폴더·파일 선택과 일부 선택 표시, 검색·분류 필터 및 파일별 담당 지정.
- Tree-sitter로 클래스·상속·필드·메서드 추출, 폴더를 기준으로 초기 시스템 구성.
- 선택된 범위 안에서 읽기 도구를 호출하는 Gemini 에이전트. 선택한 방식과 이유를 중심으로 간결한 회고 문단과 코드 위치 제공.
