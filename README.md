...
**로컬 MVP 검증을 마치고 공개 배포를 준비하고 있습니다.** 저장소 연결 → 코드 분석 → Gemini 설명 → 코멘트 반영 → 버전 검토 → Markdown 다운로드까지 동작합니다. 아직 공개 서비스 URL은 없습니다.
...
- 로컬 자동 테스트 **26개 통과**, PostgreSQL 전용 1개는 CI에서 실행. Ruff 및 TypeScript/Vite 빌드 통과.
- 실제 공개 샘플의 프로젝트 코드 **112개 수집·분석**. 데스크톱·390px 모바일, 코멘트 저장, 새로고침 유지, 검토, Markdown 다운로드 흐름 통과.
- 실제 Gemini `gemini-3.5-flash-lite`로 Machine 설명 생성 및 코멘트를 반영한 후속 버전 저장 성공. 수정 본문 665자, 코드 근거 4개. 이전 버전도 유지.
- SQLite 마이그레이션 적용 및 스키마 차이 검사 통과.
- Linux Docker 실행, PostgreSQL 실서비스, 외부 기기 접속은 **미검증**. 공개 서비스는 아직 만들지 않았습니다.
...
근거 검증은 파일·줄·버전과 인용 형식을 확인합니다. 문장의 의미와 실제 개발 의도까지 자동으로 참임을 보장하지 않으므로 사용자 검토가 필요합니다. 비공개 저장소, 계정 로그인·기기 간 동기화, 장기 보존·완전 삭제 정책과 공개 운영용 IP 제한은 후속 범위입니다.

다음 단계는 사용자 로컬 검토 → 설명·분류·UI 보완 → 무료 배포 연결 및 외부 접속 검증입니다. 실사용 후기는 사용자가 직접 경험한 내용으로 작성합니다.
